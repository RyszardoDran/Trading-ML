...
class TestPredictSequence:
    ...
    def test_predict_returns_consistent_schema_on_trend_filter(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        model = self._DummyModel(proba=0.9)
        ...
        assert result["entry_price"] == pytest.approx(float(candles_m5["Close"].iloc[-1]))
        assert result["m1_candles_analyzed"] == len(candles_m1)
        assert result["m5_candles_generated"] == len(candles_m5)
